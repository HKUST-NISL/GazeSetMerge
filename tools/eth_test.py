...
def readfolder(data, specific=None, reverse=False):
    ...
    data.landmark = [os.path.join(data.landmark, j) for j in ldm_folder]
...
def encode_data(data):
    ...
    face_path = os.path.join(test_dir, data.face)

    line = '%s %d %s %f,%f ' % (type, subid, face_path, head[0], head[1])
...
if __name__ == '__main__':
    ...
    all_path = './data/GazeSetsPng/eth_test.label'

    source = edict()
    source.image = "./data/GazeSetsPng/ETH-Gaze/Image/test"
    source.label = "./data/GazeSetsPng/ETH-Gaze/Label/test.label"
    source.landmark = "./data/GazeSetsPng/ETH-Gaze/Landmark/test.landmark"
    source.header = True
    source.name = 'eth'
