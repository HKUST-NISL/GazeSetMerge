...
class MixVisionTransformer(nn.Module):
    ...
    def init_weights(self, pretrained='models/pretrained/mit_b0.pth'):
...
class GazeFormer(nn.Module):
    def __init__(self, with_la=False):
        super().__init__()
        self.face_backbone = mit_b0()
        self.leye_backbone = mit_b0()
        self.reye_backbone = mit_b0()


        feat_size = 256

        # multi-head attention
        self.mha = TransformerEncoding(feat_size, 4, 256, 512, out_size=128)
    ...
    def forward(self, data):
        ...
        x_leye = self.face_backbone(left_eye).view(B, 1, -1)
        x_reye = self.face_backbone(right_eye).view(B, 1, -1)

        x_seq = torch.cat([x_leye, x_reye, x_face], dim=1)
        x_seq = self.mha(x_seq)
        x_feat = self.fc_feat(x_seq.view(B, -1))
...
if __name__ == '__main__':
    ...
    x = (torch.rand(batch_size, 3, in_size, in_size).cuda(), 
        torch.rand(batch_size, 3, in_size, in_size).cuda(), 
        torch.rand(batch_size, 3, in_size, in_size).cuda())
